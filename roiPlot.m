function roiPlot(roisVar,paramsVar)
 
if nargin ==0
    clearvars 
    close all
    clc
    roisFile = uigetfile('*.mat','Select mat file containing processed rois... ');
    try
        load(roisFile);
    catch
...
if settings.plotSummaryTC
    ...
    if settings.saveFigs
        for i = 1:length(f)
            ...
            suptitle([rois.fname(1:end-4), ' (pg. ', num2str(i), ')']);
            if strcmp(settings.subROIs,'All')
                saveName = [rois.fname(1:end-4) '-allTC-', num2str(i)];
            else
                saveName = [rois.fname(1:end-4) '-filtTC-', num2str(i)];
            end
...
if strcmp(plotFiltTC,'Yes')
    ...
    if settings.saveFigs
        for i = 1:length(fPlotTC)
            saveName = [rois.fname(1:end-4) '-TC-ROI_', num2str(rois.filtROI(i))];
...
if settings.plotOverlay
    ...
    try
        ...
        foo = dir(img);
        if isempty(foo)
            fprintf('Select jpg of movie projection for current roi file:\n%s', rois.fname);
    ...
    if settings.filtRoiFlag
        ...
        zOSI = zscore(rois.OSI);
        zOSIidx = find(zOSI>2);
        selected = rois.OSI(zOSIidx);
        if isempty(selected)
            selected = rois.Label;
            selectROIs = imageJrois(selected);
        end
    ...
    if settings.plotOSI
        ...
        maxVal = round(max([0.15,round(roisVals,2)]),3);
        [f(1)] = labelROIs(img, selectROIs, settings.roiType, round(roisVals,3), maxVal);
        colormap('jet');
        myColorbar(maxVal);
        ...
        % Save figure...
        if settings.saveFigs
            saveName = [rois.fname(1:end-4) '-OSImap (', settings.subROIs,')'];
            saveas(gcf,saveName,'eps2c');
            saveas(gcf,saveName,'jpeg');
    ...
    if settings.plotPO
        ...
        % Save figure...
        if settings.saveFigs
            saveName = [rois.fname(1:end-4) '-POmap (', settings.subROIs,')'];
            saveas(gcf,saveName,'eps2c');
            saveas(gcf,saveName,'jpeg');
