...
[settings, button] = settingsdlg(...
    ...
    {'Filter ROIs?','filtRoiFlag'}                      ,[false true]                           ,...
...
if settings.runRelAnalysis
    ...
    rois.sigNMResp = pvals;
end
if settings.filtNMflag
    % Calculate p value for responses to NM
    for ROI = 1:NumROIs
        cur = rois.NatMovies(ROI,:);
        foo = reshape(cur,durFrames_NM,[]);
...
if settings.runOriAnalysis
    ...
    for ROI = 1:NumROIs
        ...
        for ori = 1:Params.numOris
            ct = 0;
            for ind = ori : Params.numOris : numTrials_Grats
                ct = ct+1;
        ...
        rois.normOriResp(ROI) = (max(mean(rois.ONResponse{ROI},2)) - (mean(mean(rois.OFFResponse{ROI},2))) / (mean(mean(rois.OFFResponse{ROI},2)))  );
...
if settings.plotTableFlag
    ...
    for i = 1:NumROIs
        ...
        tableData(i,3) = rois.sigNMResp(i);
        tableData(i,4) = rois.OSI(i);
        tableData(i,5) = rois.PO(i);
        tableData(i,6) = rois.DSI(i);
        tableData(i,7) = rois.sigOriResp(i);
        tableData(i,8) = rois.GOF(i);
    end
    supTitleText = [rois.fname(1:end-9),' (All ROIs)'];
    if settings.filtRoiFlag
        ...
        for j = NumROIs+2:size(tableData,1)
            ...
            tableData(j,3) = rois.sigNMResp(filtROI(k));
            tableData(j,4) = rois.OSI(filtROI(k));
            tableData(j,5) = rois.PO(filtROI(k));
            tableData(j,6) = rois.DSI(filtROI(k));
            tableData(j,7) = rois.sigOriResp(filtROI(k));
            tableData(j,8) = rois.GOF(filtROI(k));
        end
        supTitleText = [rois.fname(1:end-9), ' (', filtStr, ')'];
        rois.filtROI = filtROI;
    end
    
    % Set column titles and formats...
    colNames = {'ROI' 'Reliability' 'NM Resp (p)' 'OSI' 'PO' 'DSI' 'Grat Resp (p)' 'GOF'};
    colForm = { 'numeric', 'numeric', 'numeric','numeric','numeric','numeric','numeric','numeric'};
    ...
    tableName = [rois.fname(1:end-9) '-fTable'];
...
if settings.saveROIflag
    fprintf('Saving...\n');
    fprintf('Saving...rois...\n');
    saveName = [rois.fname(1:end-4),'.mat'];
